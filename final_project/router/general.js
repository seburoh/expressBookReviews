...
function findBooksBy(field, value, library = books) {
  return Object.values(library).filter(book => strEq(book[field], value));
}
...
public_users.get('/', async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: "Error reading books" });
  }
});
...
public_users.get('/author/:author', async (req, res) => {
  ...
  try {
    ...
    const results = findBooksBy("author", author, data);
...
public_users.get('/title/:title', async (req, res) => {
  ...
  try {
    ...
    const results = findBooksBy("title", title, data);
